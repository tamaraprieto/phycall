...
MapMutations <- function(patient, genome=F, suffix2=""){
    ...
    if (isTRUE(genome)) {
      ...
      ad_depth_table <- data.table::fread(paste0(mydir,"/RESULTS/isec_somatic-germline/FILTERED/MutationMapping/","AD",suffix2,".bed"), header = T)  %>%
      ...
      # Bed of the genomic mutations
       ...
       bed <- data.table::fread(paste(mydir,"/RESULTS/isec_somatic-germline/FILTERED/",tag,".bed",sep=""), col.names = c("CHROM","START","END","Type","Gene"), sep=" ", na.strings = "NA") %>%
...
CreateBedFileMutationInPhylogeny <- function (patient, suffix=".genome"){
  ...
  unlist(strsplit(tree_distance@data$mutList,split = ",")) %>%
    ...
    write.table(
                  file=paste0(mydir,"/RESULTS/isec_somatic-germline/FILTERED/VariantSitesCellPhyAfterMutationMapping",suffix,".bed"),
                       quote = F, row.names = F,
                       col.names = F, sep = "\t")
}
