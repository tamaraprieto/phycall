...
MutationMappingTreeMut <- function(tree, mtr, depth, myoutgroup="t17",
                                   ...
                                   outputdir=".", filename="TreeMut.png"){
...
MapMutations <- function(patient, genome=F, suffix2=""){

      mydir=paste0("/gpfs/commons/groups/landau_lab/tprieto/BioSkryb/data/WGS/",patient)
  
      # Load the tree
        tree_final <- read.tree(paste(mydir,"/RESULTS/CellPhy-03/CellPhy.GT10+FO+E.nobulks.noCNVs.raxml.bestTree",sep=""))
    ...
    if (isTRUE(genome)) {
      ...
      ad_depth_table <- data.table::fread(paste0(mydir,"/RESULTS/isec_somatic-germline/FILTERED/MutationMapping/","AD.withchrX",suffix2,".bed"), header = T)  %>%
      ...
      bed <- data.table::fread(paste(mydir,"/RESULTS/isec_somatic-germline/FILTERED/VariantSitesForPhylogeny.hg38_multianno.nobulks.bed",sep=""), col.names = c("CHROM","START","END","Type","Gene"), sep=" ", na.strings = "NA") %>%
...
CreateBedFileMutationInPhylogeny <- function (patient, suffix=".genome"){
  ...
  tree_distance <- readRDS(file = paste0(mydir,"/RESULTS/isec_somatic-germline/FILTERED/MutationMapping/",
                                         "TreeMut",suffix,".withchrX",".rds"))
  unlist(strsplit(tree_distance@data$mutList,split = ",")) %>%
    ...
    write.table(
      file=paste0(mydir,"/RESULTS/isec_somatic-germline/FILTERED/VariantSitesForPhylogeny.hg38_multianno.nobulks",suffix,".withchrX",".bed"),
      quote = F, row.names = F,
      col.names = F, sep = "\t")
}


MapMutations(patient = patient,genome = T)
