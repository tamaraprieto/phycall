...
# For patients
VCF=${WORKDIR}/HaplotypeCaller.${PATIENT}.vqsr.heterozygous.vcf.gz
...
mkdir -p ${WORKDIR}/recall/
bcftools +fill-tags $VCF \
	--threads 4 \
	-Oz -o ${WORKDIR}/recall/VariantSitesRecall.unfiltered.vcf.gz \
	-- \
...
bcftools filter --include "NS>=${num_half_samples}"  \
	--threads 4 \
        ${WORKDIR}/recall/VariantSitesRecall.unfiltered.vcf.gz | \
	awk '{if (length($5)==1){print $0}else if ($0~/^#/){print $0}}' | \
	bgzip -c \
        > ${WORKDIR}/recall/VariantSitesRecall.unfiltered2.vcf.gz
...
bcftools view \
	--threads 4 \
	${WORKDIR}/recall/VariantSitesRecall.unfiltered2.vcf.gz | grep  -v "#" | \
        awk '{print $5}' | \
        paste -d"\t" $PATIENT".tmp" - | \
	awk 'BEGIN{print "ABC\tM"}{print $0}' | \
        paste -d"\t" ${WORKDIR}/recall/$BED - | \
	...
	sed 's/AB>MC/ABMC/' \
	> ${WORKDIR}/recall/${BED}2
rm $PATIENT".tmp"

mv ${WORKDIR}/recall/${BED}2 ${WORKDIR}/recall/$BED

# Used the bed file to annotate the vcf
echo "> Create the tsv tab with annotations"
awk 'BEGIN{printf "#"}{print $0}' ${WORKDIR}/recall/${BED} > \
         ${WORKDIR}/recall/SC.annotations.tab
bgzip -c ${WORKDIR}/recall/SC.annotations.tab > \
	${WORKDIR}/recall/SC.annotations.tab.gz
tabix -c 1 -f -b 2 -0 -e 3 \
	--comment '#' \
	${WORKDIR}/recall/SC.annotations.tab.gz

grep "^chrom" ${WORKDIR}/recall/${BED} | sed 's/#//' | sed 's/\t/\n/g' > newfilters.txt
...
bcftools annotate \
	--threads 4 \
       --annotations ${WORKDIR}/recall/SC.annotations.tab.gz \
       -h <(echo '##INFO=<ID='${a}',Number=1,Type=Float,Description="Mean VAF of cells showing at least one alternative read">\n') \
       -c CHROM,FROM,TO,-,${a},-,-,-,-,-,- \
       --output ${WORKDIR}/recall/VariantSitesRecall.unfiltered3.vcf \
       ${WORKDIR}/recall/VariantSitesRecall.unfiltered2.vcf.gz
...
bcftools annotate \
	--threads 4 \
       ...
       -c CHROM,FROM,TO,-,-,${b},-,-,-,-,- \
       --output ${WORKDIR}/recall/VariantSitesRecall.unfiltered4.vcf \
       ${WORKDIR}/recall/VariantSitesRecall.unfiltered3.vcf
...
bcftools annotate \
	--threads 4 \
       ...
       -c CHROM,FROM,TO,-,-,-,${c},-,-,-,- \
       --output ${WORKDIR}/recall/VariantSitesRecall.unfiltered5.vcf \
       ${WORKDIR}/recall/VariantSitesRecall.unfiltered4.vcf
...
bcftools annotate \
	--threads 4 \
       ...
       -c CHROM,FROM,TO,-,-,-,-,${d},-,-,- \
       --output ${WORKDIR}/recall/VariantSitesRecall.unfiltered6.vcf \
       ${WORKDIR}/recall/VariantSitesRecall.unfiltered5.vcf
...
bcftools annotate \
	--threads 4 \
       ...
       -c CHROM,FROM,TO,-,-,-,-,-,${e},-,- \
       --output ${WORKDIR}/recall/VariantSitesRecall.unfiltered7.vcf \
       ${WORKDIR}/recall/VariantSitesRecall.unfiltered6.vcf
...
bcftools annotate \
	--threads 4 \
       ...
       -c CHROM,FROM,TO,-,-,-,-,-,-,${f},- \
       --output ${WORKDIR}/recall/VariantSitesRecall.unfiltered8.vcf \
       ${WORKDIR}/recall/VariantSitesRecall.unfiltered7.vcf
...
bcftools annotate \
	--threads 4 \
       ...
       -c CHROM,FROM,TO,-,-,-,-,-,-,-,${g} \
       --output ${WORKDIR}/recall/VariantSitesRecall.unfiltered9.vcf \
       ${WORKDIR}/recall/VariantSitesRecall.unfiltered8.vcf
...
bcftools filter \
	--threads 4 \
	--include "${a}>0.35"  \
	${WORKDIR}/recall/VariantSitesRecall.unfiltered9.vcf \
        > ${WORKDIR}/recall/VariantSitesRecall.vcf
